...
(() => {
  ...
  var SlideController = class {
    ...
    init() {
      ...
      const wrapper = document.querySelectorAll("#main-section-wrapper");
    ...
    handleSectionsIntersection(entries, observer) {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          console.log(' xxxx Inside ', entry.target, observer);
          this.section_wrapper_intersection = true;
          this.activeSlide = parseInt(entry.target.getAttribute('data-info-section'));
          console.log('asdasdasd ', entry.target.getAttribute('data-info-section'));
          //set active dot
          console.log(this);
          this.removeClassFromAllDots();
          this.dots.forEach((dot) => {
            if (dot.id.charAt(dot.id.length - 1) == this.activeSlide.toString()) {
    ...
    */
     ...
     */
    slideAnim(e) {
      if (!this.section_wrapper_intersection) {
        this.enableScroll();
        return;
      }
      if (!this.toggleState) {
